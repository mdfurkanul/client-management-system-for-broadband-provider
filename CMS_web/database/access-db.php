...
Class DoWork extends conDB {
	...
	function insertRow($tbl_col_array,$table_name,$value_array){
		...
		$query="INSERT INTO ".$table_name." (".$column.") VALUES (".$col_ind.")";
		$this->QuerySQL($query,$value_array);
		return $this->output;
	} 
	
	function updateData($tbl_col_array,$table_name,$where_string,$value_array){
		
		$column='';
		$col_ind='';
		$L=sizeof($tbl_col_array)-1;
		for($i=0; $i<=$L; $i++){
			$column.=trim($tbl_col_array[$i]."=".$value_array);
			if($i != $L){
				$column.=','; 
			} 
		}
		$query="UPDATE ".$table_name." SET ".$column." where".$where_string;
		$this->QuerySQL($query,$value_array);
		return $this->output;
	} 
	...
	function unameMatch($tbl_col_array,$where_string,$value_array){ 		
		$column_name = ''; 
		$L=sizeof($tbl_col_array)-1;
		$table_name = $tbl_col_array[0];
		$this->output['rowFound']=false;
